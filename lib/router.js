  ...
  router.use = function(one, two) {
    if(typeof one === "function")
      ...
      router._push(function*(next) {
        var newParams = match(this.request.path); // Match the path against the route

        if(newParams !== false) {
          // The path matched!
          debug("matched " + this.request.path + " with " + re);
          var prevPath = this.request.path; // Save the "original" path
    ...
    router[el] = function(route) {
      ...
      router._push(function*(next) {
        ...
        if(params !== false && this.request.method.toLowerCase() === el) {
          ...
          debug("matched " + this.request.method + " " + this.request.path
          ...
          debug("didnt match " + this.request.method + " " + this.request.path +
            " with " + el.toUpperCase() + " " + route);
